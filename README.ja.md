...
既に開いているセッションのフックはセッション終了まで残ります（張り直しで完全反映）。
...
- 調整は**右クリック→「設定…」**のスライダーから（サイズ / 散歩までの秒数 / プレミア間隔 / うろうろ開始秒数）。
  config.json直編集派は `walk_after_sec` / `premium_min_sec` / `premium_max_sec` / `roam_after_sec` / `scale`
...
- **有効になるのは新しいセッションから**（セッションを張り直してください）
...
```
buddy/
  .claude/settings.json     # このプロジェクト用フック配線（生成物・git管理外）
