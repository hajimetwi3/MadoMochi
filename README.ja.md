...
- macOS：未対応ですが、**未検証の実験版**を [experimental/macos/](experimental/macos/)
  に置いています（試す前に [experimental/macos/README.md](experimental/macos/README.md)
  を読んでください。実機での動作報告を募集中）  
- ⚠️ **データの保存先として ~/.claude/buddy フォルダを使用します。
  他のツールが作った ~/.claude/buddy が既にある場合は、インストールしないでください
  （インストール自体はフォルダに触れませんが、バディは起動中に中の config.json などを
  自分の形式で上書きし、-PurgeData 付きのアンインストールはフォルダごと削除します）。
  将来のバージョンで専用フォルダ名への移行を予定しています。**  
...
- **以前の版から更新した人へ**：ファイルを差し替えたら、前と同じスコープで
  `install.ps1` を**もう一度実行**してください（フック配線を更新できるのは
  インストーラだけです）
